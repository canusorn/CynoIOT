...
#ifdef ESP8266
    // SDM120CT ค่า default จากโรงงานคือ 2400bps parity NONE (ถ้าตั้งมิเตอร์เป็น 9600 ไว้ก็เปลี่ยนเลขนี้ตาม)
    RS485Serial.begin(2400, SWSERIAL_8N1, MAX485_RO, MAX485_DI); // software serial สำหรับติดต่อกับ MAX485
#elif defined(ESP32)
    RS485Serial.begin(2400, SERIAL_8N1, MAX485_RO, MAX485_DI); // serial สำหรับติดต่อกับ MAX485
#endif
...
void loop()
{
    ...
    if (currentMillisPZEM - previousMillis >= 5000) /* for every x seconds, run the codes below*/
    {
        ...
        /* --- SDM120CT-40mA Modbus input registers (ค่าเป็น float32: 2 register ต่อ 1 ค่า) --- */
        // 0x0000-0x0001  Voltage (V)
        // 0x0006-0x0007  Current (A)
        // 0x000C-0x000D  Active Power (W)
        // 0x001E-0x001F  Power Factor
        // 0x0046-0x0047  Frequency (Hz)
        // 0x0048-0x0049  Import Active Energy (kWh)
        // 0x004A-0x004B  Export Active Energy (kWh)
        // 0x0156-0x0157  Total Active Energy (kWh)
        uint32_t var[8];
        float varfloat[8];

        result = node.readInputRegisters(0x0000, 2); /* read Voltage */
        disConnect();
        if (result == node.ku8MBSuccess) /* If there is a response */
        {
            var[0] = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1);
        }
        else
        {
            isError = true;
            Serial.println("Error 0x0000 reading");
        }

        result = node.readInputRegisters(0x0006, 2); /* read Current */
        if (result == node.ku8MBSuccess)
        {
            var[1] = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1);
        }
        else
        {
            isError = true;
            Serial.println("Error 0x0006 reading");
        }

        result = node.readInputRegisters(0x000C, 2); /* read Active Power */
        if (result == node.ku8MBSuccess)
        {
            var[2] = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1);
        }
        else
        {
            isError = true;
            Serial.println("Error 0x000C reading");
        }

        result = node.readInputRegisters(0x001E, 2); /* read Power Factor */
        if (result == node.ku8MBSuccess)
        {
            var[3] = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1);
        }
        else
        {
            isError = true;
            Serial.println("Error 0x001E reading");
        }

        result = node.readInputRegisters(0x0046, 6); /* read Frequency, Import, Export (0x0046-0x004B) */
        if (result == node.ku8MBSuccess)
        {
            var[7] = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1); /* Frequency */
            var[4] = (node.getResponseBuffer(2) << 16) + node.getResponseBuffer(3); /* Import */
            var[5] = (node.getResponseBuffer(4) << 16) + node.getResponseBuffer(5); /* Export */
        }
        else
        {
            isError = true;
            Serial.println("Error 0x0046 reading");
        }

        result = node.readInputRegisters(0x0156, 2); /* read Total Active Energy */
        if (result == node.ku8MBSuccess)
        {
            var[6] = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1);
        }
        else
        {
            isError = true;
            Serial.println("Error 0x0156 reading");
        }

        if (!isError)
        {
            varfloat[0] = hexToFloat(var[0]); // volt (V)
            varfloat[1] = hexToFloat(var[1]); // curr (A)
            varfloat[2] = hexToFloat(var[2]); // power (W)
            varfloat[3] = hexToFloat(var[3]); // pf
            varfloat[4] = hexToFloat(var[4]); // e1 = Import kWh
            varfloat[5] = hexToFloat(var[5]); // e2 = Export kWh
            varfloat[6] = hexToFloat(var[6]); // e3 = Total kWh
            varfloat[7] = hexToFloat(var[7]); // freq (Hz)
            ...
            Serial.println("0x" + String(var[4], HEX) + "\t" + String(varfloat[4], 6) + " kWh (Import)");
            Serial.println("0x" + String(var[5], HEX) + "\t" + String(varfloat[5], 6) + " kWh (Export)");
            Serial.println("0x" + String(var[6], HEX) + "\t" + String(varfloat[6], 6) + " kWh (Total)");
