...
#define ADDRESS 0x01
// CT ratio will be read from meter configuration registers

const char thingName[] = "pm2230";
...
uint16_t CTRATIO = 60; // Default CT ratio, will be updated from meter
...
void iotSetup()
{
    ...
    iot.setTemplate("pm2230", version); // เลือกเทมเพลตแดชบอร์ด Schneider PM2230
...
void loop()
{
    ...
    if (currentMillisPZEM - previousMillis >= 5000) /* for every 5 seconds, read from Schneider PM2230 */
    {
        bool isError = false;
        float varfloat[21];
        ...
        // First, read CT ratio from configuration registers
        uint8_t configResult = node.readInputRegisters(0x5000, 2);
        if (configResult == node.ku8MBSuccess) {
            uint16_t ctPrimary = node.getResponseBuffer(0);
            uint16_t ctSecondary = node.getResponseBuffer(1);
            if (ctSecondary > 0) {
                CTRATIO = ctPrimary / ctSecondary;
                Serial.println("CT Ratio updated from meter: " + String(CTRATIO) + " (" + String(ctPrimary) + "/" + String(ctSecondary) + ")");
            }
        } else {
            Serial.println("Warning: Could not read CT ratio from meter, using default: " + String(CTRATIO));
        }
        
        result = node.readInputRegisters(0x0000, 51); /* read Schneider PM2230 registers starting from 0x0000 */
        if (result == node.ku8MBSuccess)              /* If there is a response */
        {
            ...
            // pf (power factor registers for PM2230)
            tempdouble = (node.getResponseBuffer(20) << 16) + node.getResponseBuffer(21);
            var[9] = tempdouble;

            tempdouble = (node.getResponseBuffer(22) << 16) + node.getResponseBuffer(23);
            var[10] = tempdouble;

            tempdouble = (node.getResponseBuffer(24) << 16) + node.getResponseBuffer(25);
            var[11] = tempdouble;

            // freq (frequency register for PM2230)
            tempdouble = (node.getResponseBuffer(26) << 16) + node.getResponseBuffer(27);
            var[12] = tempdouble;

            // Display values on Serial monitor for Schneider PM2230
            // V (voltage scaling for PM2230)
            varfloat[0] = hexToFloat(var[0]) * 0.1;
            varfloat[1] = hexToFloat(var[1]) * 0.1;
            varfloat[2] = hexToFloat(var[2]) * 0.1;
            // I (current scaling for PM2230)
            ...
            // P (power scaling for PM2230)
            varfloat[6] = hexToFloat(var[6]) * 0.001 * CTRATIO;
            varfloat[7] = hexToFloat(var[7]) * 0.001 * CTRATIO;
            varfloat[8] = hexToFloat(var[8]) * 0.001 * CTRATIO;
            // to global variable
            power[0] = varfloat[6];
            power[1] = varfloat[7];
            power[2] = varfloat[8];
            // pf (power factor scaling for PM2230)
            varfloat[9] = hexToFloat(var[9]) * 0.001;
            varfloat[10] = hexToFloat(var[10]) * 0.001;
            varfloat[11] = hexToFloat(var[11]) * 0.001;
            // Freq (frequency scaling for PM2230)
        ...
        {
            isError = true;
            Serial.println("Error PM2230 voltage/current reading");
        }

        result = node.readInputRegisters(0x0030, 20);
        disConnect();
        if (result == node.ku8MBSuccess) /* If there is a response */
        {
            // ImpEnergy (import energy registers for PM2230)
            tempdouble = (node.getResponseBuffer(0) << 16) + node.getResponseBuffer(1);
            var[13] = tempdouble;
            tempdouble = (node.getResponseBuffer(2) << 16) + node.getResponseBuffer(3);
            var[14] = tempdouble;
            ...
            // ExpEnergy (export energy registers for PM2230)
            tempdouble = (node.getResponseBuffer(8) << 16) + node.getResponseBuffer(9);
            var[17] = tempdouble;
            tempdouble = (node.getResponseBuffer(10) << 16) + node.getResponseBuffer(11);
            var[18] = tempdouble;
            ...
            // Energy scaling for PM2230 (energy is in kWh, no additional scaling needed beyond CT ratio)
            ...
            Serial.println("------ Schneider PM2230 Data ------");
        }
        else
        {
            isError = true;
            Serial.println("Error PM2230 energy reading");
        }
