/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*
 * ระบบเซ็นเซอร์วัดดินสำหรับเกษตรอัจฉริยะ
 * =================================
 * 
 * รองรับ: ESP8266/ESP32, เซ็นเซอร์ดิน 7-in-1 (ความชื้น, อุณหภูมิ, EC, pH, N, P, K)
 * ควบคุม: ปั๊มน้ำ + วาล์ว 4 ช่อง, RS485/Modbus, OLED display, WiFi, CynoIOT cloud
 * 
...
// ==========================================================================================
// เลือกรุ่นเซ็นเซอร์ (Uncomment รุ่นที่ใช้)
// ==========================================================================================
...
const char wifiInitialApPassword[] = "iotbundle"; // รหัสผ่าน AP mode
...
// ==========================================================================================
// ตัวแปร Global - สถานะ Display และ Network
// ==========================================================================================
...
// ==========================================================================================
// ฟังก์ชันจัดการ Event จาก CynoIOT
// ==========================================================================================
...
void updateSystemState()
{
    ...
    // ป้องกันปั๊มทำงานโดยไม่มีวาล์วเปิด
...
// ==========================================================================================
// อ่านค่าเซ็นเซอร์และส่งข้อมูลไป Cloud
// ==========================================================================================
void readAndSendSensorData()
{
    ...
    if (result == node.ku8MBSuccess)
    {
        float rawHumidity = node.getResponseBuffer(0) / 10.0;
        float rawTemperature = node.getResponseBuffer(1) / 10.0;
...
#elif defined(TEMP_HUMID_EC_MODEL)
    // เซ็นเซอร์วัดความชื้น + อุณหภูมิ + EC
    ...
    if (result == node.ku8MBSuccess)
    {
        float rawHumidity = node.getResponseBuffer(0) / 10.0;
        float rawTemperature = node.getResponseBuffer(1) / 10.0;
        uint32_t rawConductivity = node.getResponseBuffer(2);
    ...
    if (result == node.ku8MBSuccess)
    {
        ...
        float rawTemperature = node.getResponseBuffer(1) / 10.0;
        uint32_t rawConductivity = node.getResponseBuffer(2);
        float rawPh = node.getResponseBuffer(3) / 10.0;
        uint32_t rawNitrogen = node.getResponseBuffer(4);
        uint32_t rawPhosphorus = node.getResponseBuffer(5);
        uint32_t rawPotassium = node.getResponseBuffer(6);
